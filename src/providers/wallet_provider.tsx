...
export function Providers({ children, email }: { children: React.ReactNode, email ?: string }) {
...
const login = async () => {
    ...
    // using supabase db
    if ((await supabaseClient.auth?.getUser()).data.user?.last_sign_in_at! >= new Date().toISOString()) {
        // creating the popup windown in order to sign and create their account
    const offchain_message = "Welcome to Coysal, please sign this message to login to your account and approve TOC";
...
if (error) {
            ...
            toast.success("Check your email for the login link!, welcome to Coysal");
